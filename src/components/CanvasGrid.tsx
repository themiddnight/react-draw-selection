...
const GridCanvas: React.FC = () => {
  ...
  useEffect(() => {
    const initialGrid: Grid = Array(GRID_HEIGHT)
      .fill(null)
  ...
  const getCellFromMouse = (e: MouseEvent<HTMLCanvasElement>): Cell => {
    ...
    return {
      x: Math.min(Math.max(cellX, 0), GRID_WIDTH - 1),
  ...
  const applyRectangle = (start: Cell | null, current: Cell | null) => {
